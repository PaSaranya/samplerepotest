# Authentication Micro Service.

### This micro service offers the required APIs for authentication. APIs include OAuth, LDAP, OTP and logger

## Authentication Service API

## POST /auth/facebook, /auth/google, /auth/twitter, /auth/linkedin, /auth/custom
- These APIs will initiate the authentication to providers. This is the first API to be consumed for authentication.
- This API requires apiKey as headers. In return if apiKey is valid then a token will be sent along with next call details.
- If any prehooks are available then response json will have details of nextCall, token and channelProvider with statusCode as 303
- If no prehooks are available then response json will have the details of nextCall with status code as 302

### Request

| Headers  |                  Description                                                          |
|--------------|---------------------------------------------------------------------------------------|
| apiKey  | apiKey which will be obtained from the VCAPS of the bounded developer application    |
| datekey     | DD-MM-YYYY                                                                      |
| Content-Type | application/json                                                                |
| hashcode | Generated Hash code                                                                 |
| nonce | generated nonce                                                                        |

### Response
- With prehooks available

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |303 |
|body   | {"nextCall":"/generateOtp","channelprovider":"sendgrid","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}  |


- Without prehooks available

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |302 |
|body   | {"nextCall":"/facebook/eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}  |



## GET /{provider}/:token?callbackUrl={DEV_REDIRECTION_URL}

### APIs
- /facebook/:token?callbackUrl={DEV_REDIRECTION_URL}
- /google/:token?callbackUrl={DEV_REDIRECTION_URL} 
- /twitter/:token?callbackUrl={DEV_REDIRECTION_URL}
- /linkedin/:token?callbackUrl={DEV_REDIRECTION_URL}
- /custom/:token?callbackUrl={DEV_REDIRECTION_URL}

### Description
- Authenticates the user against requested provider and redirects back to application with a path parameter as token.
- Developer application has to redirect to any of the above APIs with token as path parameter and query parameter as callbackUrl(to where microservice has to redirect back)
- Token information needs to be retrieved from prior call (POST /auth/facebook, /auth/google, /auth/twitter, /auth/linkedin,/auth/custom)

- Developer application should follow the below steps to implement the callback route

      ```
       1. While calling API callbackUrl should have only the route without path parameter. But while defining the route in the developer application it should have path parameter as a token.

       eg. Passing a callbackUrl as http://localhost:3000/callback should have route as '/callback/:token'
      ```

- The token which is coming in the path parameter should be retained and given to the next call that is /auth/complete

### Request

| Query Param  |                  Description                                                          |
|--------------|---------------------------------------------------------------------------------------|
| callbackUrl  | callbackUrl to where Authentication service needs to redirect after authentication    |

### Response

| HTTP           |      Value                           |
|----------------|--------------------------------------|
| path parameter   | eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}   |



## POST /auth/complete
- This API should be called after authentication to a provider and it is redirected back to developer application.
- The token which is retained from the prior call should be passed as 'token' and  also 'apiKey' in headers
- In response if any post hooks are available then response json will have details of nextCall, token and channelProvider with statusCode as 303
- If no prehooks are available then response json will have the details of the loggedin user with status code as 200. This response will have accessToken, id, displayName and provider details. For twitter refreshToken will be available.

### Request

| Headers  |                  Description                                                          |
|--------------|---------------------------------------------------------------------------------------|
| apiKey  | apiKey which will be obtained from the VCAPS of the bounded developer application    |
| token  | The token which is retained from the prior call    |

### Response

- With posthooks available

| HTTP           |Value|
|----------------|--------------------------------------|
|   status code |303 |
|body   | {"nextCall":"/generateOtp","channelprovider":"sendgrid","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}  |


- Without posthooks available

| HTTP           |Value|
|----------------|--------------------------------------|
|   status code |200 |
| Body       | {"accessToken":"Rcp4xZC06HnmcFMaW866wS9KSK4bCjxBQB2UTPfVbOJH","id":"607861336036","displayName":"John Doe","provider":"facebook"}|


*Note: refreshToken will be available  for /twitter  or /custom call. Not applicable for other providers



## POST /generateOtp
- This API generates an OTP with the given combination(numeric,alpha or both) and sends the OTP to desired receipient based on the given channel and receipient details.

- If the channelProvider is sendgrid then this API requires request body parameters as {"toRecipient":"someemail@gmail.com","fromMail": "someemail@gmail.com"}

- If the channelProvider is twilio then this API requires request body parameters as {"toRecipient":"+765XXXX","fromNo": "+91XXXXXX"}

- The response json will have details of nextCall and token with statusCode as 303

### Steps to Generate (or) Skip OTP

- OTP would be either generated/skipped based on conditional hooks configured on the UI and the header values passed from the request.

- OTP would be generated:
      - If a conditional hook is added and same is passed through headers.
      - If a conditional hook is not added.

- OTP would be skipped if a conditional hook is added and not passed through headers.

### Request

- channelProvider as Sendgrid

| HTTP       | Value |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| token      | The token which is retained from the prior call should be passed as header |
| some condition | The condition should be passed here. e.g: if the condition is like "x is equals to 1"  then "x" has to be passed as header key and 1 will be value of header. |
| Body       | {"toRecipient":"someemail@gmail.com","fromMail": "someemail@gmail.com"} |


- channelProvider as Twilio

| HTTP       | Value|
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| token      | The token which is retained from the prior call should be passed as header |
| some condition | The condition should be passed here. e.g: if the condition is like "x is equals to 1"  then "x" has to be passed as header key and 1 will be value of header. |
| Body       | {"toRecipient":"+765XXXX","fromNo": "+91XXXXXX"} |

### Response

| HTTP       |  Value|
|------------|--------------------------------------------------------------------------------------------|
|body   | {"nextCall":"/validateOtp","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}  |

* Note : Default values for otpLength, otpType and otpExpiryTime are 4, numeric and 15(mins) respectively. otpType can be of numeric or alpha or alphanumeric



## POST /validateOtp
- This API validates the OTP
- In the request you have to pass otpCode in body with the token in the header parameter
- If any further hooks are available then response json will have details of nextCall, token and channelProvider with statusCode as 303
- If there are no further prehooks then response json will have the details of nextCall with status code as 302 normally it will be the redirect url to the providers.
- If no further posthooks are available then response json will have the details of the loggedin user with status code as 200. This response will have accessToken, id, displayName and provider details. For twitter refreshToken will be available.

### Request

| HTTP       |Value   |
|------------|--------------------------------------------------------------------------------------------|
| Body       | {"otpCode":"79653"}        |
| token      | The token which is retained from the prior call should be passed as header |


### Response
- If further prehooks or posthooks are available

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |303 |
|body   | {"nextCall":"/generateOtp","channelprovider":"sendgrid","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}  |


- If no further prehooks or posthooks are available

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |200 |
| Body       | {"accessToken":"Rcp4xZC06HnmcFMaW866wS9KSK4bCjxBQB2UTPfVbOJH","id":"607861336036","displayName":"John Doe","provider":"facebook"}|

## POST /webapi/generate/:param1/:param2/:param3
- This API generates an web api response  with the given configuration and sends the response back to the user.
- User have to give necessary clientID and clientSecret which will be used as the authentication for the hooks which is configured . Our microservice will be passing the clientID and clientSecret in the headers of the Web api

- All the configured values of headers, queryParams and Body will be piped to the configured web api which will be  a rest Api
- In the case of pathParams we are taking any number of path params which will be appended to the configured url.
- The response from the web api will be piped to response of our microservice.
- Details such as nextCall,token and statusCode will be passed in the headers instead of body .
### Request

| HTTP       | Value |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| clientID     | This is an optional Value. Generally provided by the client ensure valid users only connect to system |
| clientSecret| This is an optional Value. Generally provided by the client ensure valid users only connect to system |
| Body       | Required body parameters which has to be sent to the configured web api | 
| headers       | Required headers parameters which has to be sent to the configured web api |
| query       | Required query parameters which has to be sent to the configured web api |
| path       | Required path parameters which has to be sent to the configured web api |
| url       | Url that needs to be hit in order to get the content |
| method       | method can be any one of these (get,post,put). Delete will definely not be an option  |

### Response

| HTTP       |  Value|
|------------|--------------------------------------------------------------------------------------------|
|headers   | {"nextCall":"/validateOtp","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g","statusCode":303}  |
* Note : The response from the Web api will be piped to the response here

## POST /webapi/validate/:param1/:param2/:param3
- This API generates an web api response  with the given configuration and sends the response back to the user.
- User have to give necessary clientID and clientSecret which will be used as the authentication for the hooks which is configured . Our microservice will be passing the clientID and clientSecret in the headers of the Web api

- All the configured values of headers, queryParams and Body will be piped to the configured web api which will be  a rest Api
- In the case of pathParams we are taking any number of path params which will be appended to the configured url.
- The response from the web api will be piped to response of our microservice.
- Details such as nextCall,token and statusCode will be passed in the headers instead of body .
### Request

| HTTP       | Value |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| clientID     | This is an optional Value. Generally provided by the client ensure valid users only connect to system |
| clientSecret| This is an optional Value. Generally provided by the client ensure valid users only connect to system |
| Body       | Required body parameters which has to be sent to the configured web api | 
| headers       | Required headers parameters which has to be sent to the configured web api |
| query       | Required query parameters which has to be sent to the configured web api |
| path       | Required path parameters which has to be sent to the configured web api |
| url       | Url that needs to be hit in order to get the content |
| method       | method can be any one of these (get,post,put). Delete will definely not be an option  |

### Response

- If further prehooks or posthooks are available

| HTTP       |  Value|
|------------|--------------------------------------------------------------------------------------------|
|headers   | {"nextCall":"/validateOtp","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g","statusCode":303}  |

- If no further prehooks or posthooks are available

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |200 |
| Body       | {"accessToken":"Rcp4xZC06HnmcFMaW866wS9KSK4bCjxBQB2UTPfVbOJH","id":"607861336036","displayName":"John Doe","provider":"facebook"}|

## LDAP related APIs

## POST /auth/ldap
- This API will initiate the authentication to configured LDAP server. This is the first API to be consumed for LDAP authentication.
- This API requires apiKey as headers. In return if apiKey is valid then a token will be sent along with next call details.
- If any prehooks are available then response json will have details of nextCall, token and channelProvider with statusCode as 303
- If no prehooks are available then response json will have the details of nextCall and token with status code as 303

- token received in response should be used in headers for nextCall

### Request

| Headers  |                  Description                                                          |
|--------------|---------------------------------------------------------------------------------------|
| apiKey  | apiKey which will be obtained from the VCAPS of the bounded developer application    |


### Response
- With prehooks available

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |303 |
|body   | {"nextCall":"/generateOtp","channelprovider":"sendgrid","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}  |

- Without prehooks available

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |303 |
|body   | {"nextCall":"/ldap","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}  |


## POST /ldap
- Authenticates the user against configured LDAP server.
- Request headers for this API are token(received in prior call /auth/ldap API), dn and password
- If any posthooks are available then response json will have details of nextCall, token and channelProvider with statusCode as 303
- If no posthooks are available then response json will have the message and authFlag(boolean) with status code as 200

### Applying pre configured filters for fetching user profile
- Make sure 'baseDN', 'filter', 'filter attributes' and 'attributes to search' is configured from Command center
- pass userFilter parameter as true in body (Note : userFilter can accept only boolean values true or false)
- pass all the filter attributes values as body parameters


- Example for setting filters

      ```
       - Example

        1. consider the following settings set in command center
             ldapURL            : ldap://10.233.45.56/389
             baseDN             : (dc=foo, o=example)
             filter             : (&(empid=<employeeId>)(l=Seattle)(!(|(employeeType=manager)(employeeType=temporary))))
             filter attributes  : ["employeeId"]
             attributes to fetch: ["firstname","lastname"]

        2. In the above settings
             ldapURL is the URL where LDAP is configured
             baseDN is the DN under which user profile to be searched
             filter is constructed to fetch the user profile. Dynamic values should be placed in angular brackets and same needs to be given in 'filter attributes' field
             'filter attributes' are the names used in 'filter'. These names will be used as keys in body of /ldap with appropriate values.
             'attributes to fetch' are the attributes fetched in user profile as end result

        3. Example header and body of /ldap with the above settings

        - header
             {
                token    :eyJhbGciO.iJIUzI1NiIsInR5c.CI6IkpXVCJ9......
                dn       :cn=129421,dc=foo,o=example
                password :password-1
             }

        - body
              {
                   useFilter  : true,
                   employeeId : "129421"
              }

      ```



### Overriding filter through body parameter for fetching user profile
- Make sure 'baseDN' and 'attributes to search' is configured from Command center
- pass userFilter parameter as false in body (Note : userFilter can accept only boolean values true or false)
- pass all the filter parameter in body


- Example for overriding filter through body of /ldap

      ```
       - Example

        1. consider the following settings set in command center
             ldapURL            : ldap://10.233.45.56/389
             baseDN             : (dc=foo, o=example)
             filter             : (&(empid=<employeeId>)(l=Seattle)(!(|(employeeType=manager)(employeeType=temporary))))
             filter attributes  : ["employeeId"]
             attributes to fetch: ["firstname","lastname"]

        2. In the above settings all parameters are set. But in case of changing the filter it can be possible from body parameter. pass useFilter as false and mention filter in body

        3. Example header and body of /ldap with the above settings

        - header
             {
                token    :eyJhbGciO.iJIUzI1NiIsInR5c.CI6IkpXVCJ9......
                dn       :cn=129421,dc=foo,o=example
                password :password-1
             }

        - body
              {
                   useFilter  : false,
                   filter : "(&(empid=129421)(l=Seattle))"
              }

      ```

- If useFilter is not present in body then only authentication will be done and no user profile will be fetched.

### Request

| Headers  |                  Description                  |
|--------------|-------------------------------------------|
| token  | token received in prior call /auth/ldap API   |
| dn | LDAP distinguished name for user |
| password | password of the user |

      ```
       - Examples for dn
             1. cn=foo, ou=people, o=example (passing the whole)
             2. foo (passing just username)

       - dn differs from LDAP to LDAP configurations. Sometimes it requires just username/alias and sometimes it requires whole dn.
      ```


| Body  |                  Description                  |
|--------------|-------------------------------------------|
| useFilter  | It can accept boolean values either true or false  |
| filter_attribute1 | value for filter_attribute1 |
| filter_attribute2 | value for filter_attribute2.. pass all the filter attributes like this one by one in case of useFilter true |
| filter | In case of useFilter is false. pass the filter |

### Response
- With posthooks available

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |303 |
|body   | {"nextCall":"/generateOtp","channelprovider":"sendgrid","token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cvNZ2OWO78zTTItlUfl8V-XujosiP4BTzW_Ug-wQF3g"}  |

- Without posthooks available

- With useFilter as true. And all filter attributes passed. It will fetch the user profile according to filter and settings configured

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |200 |
|body   | {"firstname":"John","lastname":"Doe"} |


- With useFilter as false. And filter as body parameter. It will fetch the user profile according to filter mentioned in body and settings configured

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |200 |
|body   | {"firstname":"John","lastname":"Doe"} |



- Without useFilter in body

| HTTP           |      Value                           |
|----------------|--------------------------------------|
|   status code |200 |
|body   | {"message":"authentication successful","authFlag":true} |
